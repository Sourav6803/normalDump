...
const ExportSearchpage = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        await axios.get(`http://localhost:4002/api/${ivalue}/filter?${valState}=${query}`)
    ...
    return (
        <>
            <div className='d-flex align-items-center justify-ontent-center flex-column' style={{ height: "30vh" }}>
                <div className=' mt-3 d-flex align-items-center justify-ontent-center'>
                    <h1 className='mb-2 d-flex align-items-center justify-ontent-center text-white ' style={{ display: "block", fontSize: "32px", font: "bold" }}>Search import Export Data Of India</h1>
                </div>

                <div className="radioBtn d-flex align-items-center justify-content-center ">
                    <input className="fs-1 text-white m-2" type="radio" value="import" name="data" onChange={e => setValue(e.target.value)} />Import
                    <input className="fs-1 text-white m-2" type="radio" value="export" name="data" onChange={e => setValue(e.target.value)} />Export
                </div>
                ...
                <div className='mb-3 mt-5'>
                    <form onSubmit={handleSubmit} className="d-flex align-items-center justify-content-center mb-3">
                        ...
                        <select className="bg-warning  text-bold m-0 p-0" value={valState} name='import' style={{ height: "4.5vh", width: "6vw" }} onChange={(e) => {
                            let selectValue = e.target.value
                            setValState(selectValue)
                        }} >
                            <option value="HSN_CODE" >HSN_CODE</option>
                            <option value="HSN_CODE_DESCRIPTION" >HSN_CODE_DESCRIPTION</option>
                            <option value="IMPORTER_NAME" >COMPANY NAME</option>
                        </select>
                        <input className=" " style={{ display: "inline-block", paddingLeft: "10px", height: "30px", width: "40vw", border: "none" }} type='text' placeholder={valState} value={query} onChange={e => setQuery(e.target.value)} />
            ...
            <div>
                {currentItems.length > 0 ?
                    <div className='container '>
                        <div>
                            <h1 className='mt-3 d-flex align-items-start text-black '>Detailed Export Data Of <span style={{ display: "block", marginLeft: "10px", color: "blue", fontSize: "32px", fontWeight: "bold" }}>{query}</span> </h1>
                        </div>

                        <div className='row m-1'>
                            <div className="col m-1 " style={{ height: "20vh", borderRadius: "20px", backgroundColor: "grey" }}>
                                <h1 className="d-flex justify-content-center mt-3 text-warning font-weight-bold">{getImportData.length}</h1>
                                <h4 className='d-flex justify-content-center mt-3 text-white'>Export Shipment Records found</h4>
                            </div>

                            <div className="col m-1 bg-secondary" style={{ height: "20vh", borderRadius: "20px" }}>
                                <h2 className="d-flex justify-content-center m-3 text-white">Detailed Analysis & Trends Of: </h2>
                                <h3 className='d-flex justify-content-center m-3 text-white'> Export Of:{" "} <span className='text-warning'>  {query?.slice(0, 50)}</span></h3>
                                <div className='bg-primary mt-5' style={{ height: "30px" }}>
                                    <h2 className='d-flex justify-content-center align-items-center mt-3 text-white' >CLICK TO VIEW</h2>
                                </div>
                            </div>

                            <div className="col m-1 bg-secondary" style={{ height: "20vh", borderRadius: "20px" }}>
                                <h2 className="d-flex justify-content-center m-3 text-white">Customs Duty Of:  <span style={{ display: "block", marginLeft: "5px", color: "yellow" }}> {query}</span></h2>

                                <div className='bg-primary ' style={{ height: "30px", marginTop: "65px" }}>
                                    <h2 className='d-flex justify-content-center align-items-center mt-3 text-white' >CLICK TO VIEW</h2>
                                </div>
                            </div>

                            <div className="col m-1 bg-secondary" style={{ height: "20vh", borderRadius: "20px" }}>
                                <h2 className="d-flex justify-content-center align-items-center mt-3 text-white">Trademarks </h2>
                                <h2 className="d-flex justify-content-center align-items-center mt-1 text-white">On This Page</h2>
                                <div className='bg-primary ' style={{ height: "30px", marginTop: "35px" }}>
                                    <h2 className='d-flex justify-content-center align-items-center mt-3 text-white' >CLICK TO VIEW</h2>
                                </div>
                            </div>
            ...
            <div>
                {
                    currentItems.length > 1 ? <div className='container'>
                        <div style={{ paddingTop: "20px", paddingBottom: "20px" }}>
                            <a className='btn btn-primary btn-round' style={{ display: "inline-block", marginBottom: "0", fontWeight: "400", textAlign: "center", padding: "6px 12px", fontSize: "14px" }}>Exports Of {query} in India</a>
                            <a className='btn btn-primary btn-round ms-2' style={{ display: "inline-block", marginBottom: "0", fontWeight: "400", textAlign: "center", padding: "6px 12px", fontSize: "14px" }}>Manufactures Of {query}</a>
                            <a className='btn btn-primary btn-round ms-2' style={{ display: "inline-block", marginBottom: "0", fontWeight: "400", textAlign: "center", padding: "6px 12px", fontSize: "14px" }}>Buyers of {query} in India</a>
                        </div>
                    </div> : ""
                }
            </div>
            ...
            <div>
                {
                    currentItems.length > 0 ? <div className="result">
                        <table>
                            <thead>
                                <tr>
                                    {newKey.map((item, index) => {
                                        return (
                                            <th  scope="col" key={index}><span>{item}</span></th>
                                        )
                                    })}
                                </tr>
                            </thead>
                            <tbody>
                                {
                                    currentItems?.length > 0 ? <UserData data={currentItems} /> : <div>No data found </div>
                                }
                            </tbody>
                        </table>
